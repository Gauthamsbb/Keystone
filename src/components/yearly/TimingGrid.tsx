...
// 13 groups of 4 weeks = 52 weeks total
...
// Light-mode colors keyed by time-of-day bucket
function dayCellStyle(timeMins: number | null): { bg: string; text: string } {
  if (timeMins === null) return { bg: 'bg-gray-100', text: 'text-gray-300' };
  const h = timeMins / 60;
  if (h < 6)  return { bg: 'bg-indigo-100',  text: 'text-indigo-600' };
  if (h < 9)  return { bg: 'bg-sky-100',     text: 'text-sky-700' };
  if (h < 12) return { bg: 'bg-amber-100',   text: 'text-amber-700' };
  if (h < 15) return { bg: 'bg-orange-100',  text: 'text-orange-700' };
  if (h < 18) return { bg: 'bg-orange-200',  text: 'text-orange-800' };
  if (h < 21) return { bg: 'bg-violet-100',  text: 'text-violet-700' };
  return       { bg: 'bg-indigo-200',  text: 'text-indigo-700' };
}

// Short time label: "8:30" (24-h, no seconds)
function timeShort(timeMins: number): string {
  const h = Math.floor(timeMins / 60);
  const m = timeMins % 60;
  return `${h}:${m.toString().padStart(2, '0')}`;
}

export function TimingGrid({ habits, entries, year, streaks }: TimingGridProps) {
  ...
  const timingHabits = habits.filter((h) => h.timing_field_id !== null);
    ...
    <div className="rounded-2xl overflow-hidden shadow-[0_2px_12px_rgba(124,58,237,0.08)] border border-violet-100 bg-white">
      <div className="overflow-x-auto">
        <div className="min-w-max">
          ...
          {/* ── Header: month → week number → M T W T F S S ─── */}
                  ...
                  {group.map((wk) => {
                    ...
                    return (
                      <div key={wk} className="flex flex-col items-center">
                        <span
                          className={`text-[8px] font-semibold mb-0.5 leading-none
                            ${isCurWk ? 'text-violet-600' : 'text-violet-300'}`}
                        >
                          W{wk}
                        </span>
          ...
          {/* ── Habit rows ─────────────────────────────────────── */}
          {timingHabits.length === 0 ? (
            <div className="px-4 py-8 text-center text-gray-400 text-sm">
              No habits with a Timing field configured.{' '}
              <span className="text-gray-400">Edit a habit and set a Time field as the Timing field.</span>
            </div>
          ) : (
            timingHabits.map((habit, hi) => {
              const streak = streakMap[habit.id];
              const dailyMap = buildDailyMap(entries, habit.id);
              const timingFieldId = habit.timing_field_id!;
                        ...
                        return (
                          <div key={wk} className="flex gap-px">
                            {DAY_LETTERS.map((_, di) => {
                              ...
                              const timeMins =
                                entry?.values.find((v) => v.field_id === timingFieldId)?.time_mins ?? null;
                              const isToday = dateStr === todayStr;
                              const { bg, text } = dayCellStyle(timeMins);
                              const ring = isToday
                              ...
                              return (
                                <div
                                  ...
                                  title={`${dateStr}: ${timeMins !== null ? timeShort(timeMins) : 'no entry'}`}
                                >
                                  {timeMins !== null && (
          ...
          <div className="flex flex-wrap items-center gap-4 px-3 py-2 bg-violet-50 border-t border-violet-100">
            <span className="text-[10px] text-violet-400 font-medium uppercase tracking-wider">Time of day</span>
            {[
              { bg: 'bg-indigo-100',  label: '12–6am'   },
              { bg: 'bg-sky-100',     label: '6–9am'    },
              { bg: 'bg-amber-100',   label: '9am–12pm' },
              { bg: 'bg-orange-100',  label: '12–3pm'   },
              { bg: 'bg-orange-200',  label: '3–6pm'    },
              { bg: 'bg-violet-100',  label: '6–9pm'    },
              { bg: 'bg-indigo-200',  label: '9pm–12am' },
            ].map(({ bg, label }) => (
